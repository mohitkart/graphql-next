...
export const allApi = async ({ url = '', method = 'get', payload, params, controllerRef }: { url: string, method: 'get' | 'put' | 'post' | 'delete', params?: any, payload?: any, controllerRef?: any }) => {
    ...
    try {
        let res: any = null;
        if (method == 'get' || method == 'delete') res = await (api as any)[method]?.(url, { params: params, signal: controller.signal })
        else res = await (api as any)[method]?.(url, payload, { params: params, signal: controller.signal })
        return res.data
    } catch (err) {
